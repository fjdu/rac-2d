...
def make_plot(coords_1, LUT, dict_1, minflux, maxflux, fig_fname,
...
def task1():
  res_dir = '/n/Users/fdu/now/grid_20150703/results'
  RT_dir = 'H2O_ground/'
  images_dir = 'images'
  LUT_file = os.path.join('/n/Users/fdu/now/grid_20150703/config_files/LUT.dat')
  dV = 1.0
  
  fPatterns = {'$1_{10}-1_{01}$': ['H2O_ground', 'line_00001_00212_00001_5.56936E+11_0.00.fits'],
               '$1_{11}-0_{00}$': ['H2O_ground', 'line_00002_00437_00001_1.11334E+12_0.00.fits'],
               '$3_{12}-2_{21}$': ['H2O_1', 'line_00002_00456_00001_1.15313E+12_0.00.fits'],
               '$3_{12}-3_{03}$': ['H2O_1', 'line_00001_00428_00001_1.09736E+12_0.00.fits']}
  
  dobs, sources, transitions = load_latex_tab('/n/Users/fdu/now/grid_20150703/tab_for_python.tab')
  ...
  for trans in fPatterns.keys():
      ...
      coords_1, dict_1, minflux, maxflux = make_data(LUT, key_flux = 'INTFLUXL')
      mdust_s = np.array([c[1] for c in coords_1])
  
      fig_fname = de_latexmath(trans) + '_0.pdf'
  
      make_plot(coords_1, LUT, dict_1, minflux, maxflux, fig_fname,
                dobs = dobs, sources = sources, trans = trans, mdust_s = mdust_s)
  return
...
def task2():
    ...
    #tab_file = 'tab_for_python_Oct04.tab'
                        ...
                        if len(d_collect) > 0:
                            for d_c in d_collect:
                                ...
                                if d_c[2] == 0.0: # Only upper limit
                                    ...
                                    ax.text(d_c[0]+0.1, d_c[1]/3, d_c[3], rotation="vertical", size="small", ha="left", va="center")
                                else:
                                    #ax.plot([d_c[0]], [d_c[2]],
                                    ...
                                    ax.text(d_c[0]+0.1, d_c[2], d_c[3], rotation="vertical", size="small", ha="left", va="center")
...
def task3():
    ...
    toKelvin = False
    ylabstr = r'$\int T_{} dV$ (K km s$^{{-1}}$)' if toKelvin else r'W m$^{-2}$'

    fPatterns = {'1-0': ['HD', 'line_00001_00002_00001_2.67499E+12_7.00.fits'],
                 '2-1': ['HD', 'line_00002_00004_00001_5.33156E+12_7.00.fits']}
    ...
    for trans in fPatterns.keys():
        LUT = load_lut(res_dir, fPatterns[trans][0], images_dir, fPatterns[trans][1], LUT_file,
                       dV = dV, key_flux = 'INTFLUXL', nrows=72, toKelvin=toKelvin)
        LUT_s.update({trans: LUT})
    ...
    for trans in fPatterns.keys():
        ...
        for id2g, d2g in enumerate(d2gs):
            for imdust, mdust in enumerate(mdusts):
                ax = make_sub_axis(fig, nmdust, nd2g, imdust, id2g,
                        pansepxfrac=0.04, pansepyfrac=0.0,
                        labelfontsize=labelfontsize, tickfontsize=textsize,
                        xtitle = 'Stellar type', ytitle = ylabstr,
                        xticks = [0,1,2,3,4,5],
                        xticklabels = ['M', 'K', 'G', 'F', 'A', 'B'],
                        yscale = 'log' if maxy/miny>30 else 'linear',
                        xRange = (-0.5, 5.5),
        ...
        fig_fname = os.path.join(fig_dir, de_latexmath(trans) + '_HD.pdf')
...
def task4():
    ...
    LUT_s = {}
    for trans in fPatterns.keys():
        LUT0 = load_lut(res_dir, fPatterns[trans][0], images_dir, fPatterns[trans][1], LUT_file,
                       dV = dV, key_flux = 'INTFLUXL', nrows=72, toKelvin=toKelvin)
        for mdust in {x['mdust'] for x in LUT0}:
            LUT = filter(lambda x: x['mdust'] == mdust, LUT0)
            LUT = groupby(lambda x: x['spectralType'], LUT)
            LUT = {k: orderby(lambda x: x['d2g'], LUT[k]) for k in LUT.keys()}
            for k in LUT.keys():
                print k
                for i in LUT[k]:
                    print i['d2g'], i['INTFLUXL']
            fig = pl.figure(figsize = figsize)
            #
            lut0 = LUT[LUT.keys()[0]][0]
            pl.figtext(0.1, 1,
                    'Molecule: ' + lut0['molname'].strip() + \
                        ':\t' + \
                        trans.replace('-', r'$\rightarrow$') + \
                        #lut0['qnum'].replace('->', r'$\rightarrow$') + \
                        '\n' + 
            ...
            ax = fig.add_axes((0.1,0.1,0.89,0.89),
                    xlabel='Dust-to-gas mass ratio', ylabel='Flux (W m$^{-2}$)',
                    autoscalex_on=False, autoscaley_on=False,
                    xscale='log', yscale='log',
                    xlim=(5e-3, 2), ylim=(1e-21, 1e-15)
                )
            mdust_str = my_script.num2str4fig(mdust)
            ax.text(1e-1, 3e-16, r'$M_{{\rm dust}} =$ ' + mdust_str + ' $M_\odot$',
                fontdict={'horizontalalignment': 'center', 'verticalalignment': 'bottom'},
                fontsize=textsize)

            #spectralType_s = LUT.keys() 
            spectralType_s = [_ for _ in 'OBAFGKM' if _ in LUT.keys()]
            for iST, specType in enumerate(spectralType_s):
                x = [item['d2g'] for item in LUT[specType]]
                y = [item['INTFLUXL'] for item in LUT[specType]]
                print x, y
                ax.plot(x, y, color = lfd.mycolors[iST], label=specType, marker='o', linewidth=5)

            ax.legend(loc='best', prop={'size':15},
                      fancybox=False, shadow=False, ncol=1)
            fig_fname = os.path.join(fig_dir, de_latexmath(trans) + '_HD_' + '{:.2e}'.format(mdust) + '.pdf')
            print 'Saving figure', fig_fname
            pl.savefig(fig_fname, bbox_inches='tight')
...
def task5():
    ...
    import my_script
    # HD
    grid_dir = '/nfs/ebergin1/fdu1/work/grid_20150703_redo'
    res_dir = os.path.join(grid_dir, 'results')
    fig_dir = os.path.join(grid_dir, 'figures')
    images_dir = 'images'
    LUT_file = os.path.join(grid_dir, 'config_files/LUT.dat')
    ...
    fPatterns = {'1-0': ['HD_redo', 'line_00001_00002_00001_2.67499E+12_7.00.fits'],
                 '2-1': ['HD_redo', 'line_00002_00004_00001_5.33156E+12_7.00.fits']}
    ...
    for trans in fPatterns.keys():
        ...
        for mdust in {x['mdust'] for x in LUT0}:
            ...
            #
            ax = fig.add_axes((0.1,0.1,0.89,0.89),
                    xlabel='Dust-to-gas mass ratio', ylabel='Flux (Jy)',
                    autoscalex_on=False, autoscaley_on=False,
                    xscale='log', yscale='log',
                    xlim=(5e-3, 2), ylim=(1e-1, 1e4)
                )
            mdust_str = my_script.num2str4fig(mdust)
            ax.text(1e-1, 3e-16, r'$M_{{\rm dust}} =$ ' + mdust_str + ' $M_\odot$',
                fontdict={'horizontalalignment': 'center', 'verticalalignment': 'bottom'},
            ...
            for iST, specType in enumerate(spectralType_s):
                x = [item['d2g'] for item in LUT[specType]]
                y = [0.5*(item['flux_vec'][0]+item['flux_vec'][-1]) for item in LUT[specType]]
                print x, y
                ax.plot(x, y, color = lfd.mycolors[iST], label=specType, marker='o', linewidth=5)

            ax.legend(loc='best', prop={'size':15},
                      fancybox=False, shadow=False, ncol=1)
            fig_fname = os.path.join(fig_dir, de_latexmath(trans) + '_HD_continuum_flux_' + '{:.2e}'.format(mdust) + '.pdf')
            print 'Saving figure', fig_fname
            pl.savefig(fig_fname, bbox_inches='tight')
