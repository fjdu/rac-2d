...
def least_square_fit_cube(cb, x = None, deg=1):
    """
    Fit the baseline of a data cube.
    """
    nx, ny, nz = cb.shape
    if x is None:
        x = np.arange(0, nx)
...
class cube():
    ...
    def remove_cube_baseline(self, n_edgechannel=2):
        ...
        # Note that np.polyval and np.polynomial.polynomial.polyval use
        # different order for the polynomial coefficients.
        a = a[::-1]
        xnew = np.arange(0, nx)
        for i in xrange(ny):
            for j in xrange(nz):
    ...
    def draw_sum_channels(self, pdfname, returnfig=False, returnax=False, savepdf=True, logscale=False, notext=False,
                drawPeak=False,
                pos = (0.1,0.1,0.85,0.85), fig=None, ax=None, vmin=None, vmax=None, noColorbar=False, ytextoff=False,
                cax=None, orientation='horizontal', cbar_ticks=None, pos_angle=None,
                axisUnit='AU', xRange=None, yRange=None, rescaleTo=None, cbarcolor='black',
                gaussianfit=False,
            ):
        if logscale:
...
def draw_an_image(data, x, y, pdfname, tag='', tagcolor='white',
                  ...
                  cmap = None, badcolor=None, cax=None, orientation='horizontal', cbar_ticks=None, cbarcolor='black',
    ...
    if (not noColorbar) and cax:
        plt.colorbar(C, cax=cax, orientation=orientation, ticks=cbar_ticks)
    ...
    set_axis_format(ax, xscale='linear', yscale='linear', labelfontsize=fontsize, tickfontsize=fontsize*0.8,
                    majorgridon=True, minorgridon=True, graygrid=True)
    if ytextoff:
...
if __name__ == '__main__':
 ...
 #  for L in [
 #              {
 #               'modelname': '20150609_C2H_5a_4a_2d_8cl_B6l_25',
 #               'line_dir':  'C2H_2_NLTE',
 #               'linefname': 'line_00001_00030_00001_3.49312E+11_7.00.fits',
 #               'pdfname':   '/n/Users/fdu/now/specGrid_C2H_1.pdf',
 #              },
 #           ]:
 #      task14(x_range=(-40,40), y_range=(-40,40), **L)
    for model in [
                    '20150609_C2H_5a_4a_2d_8cl_B6l_42',
                 ]:
